...
function main(names::AbstractVector=["all"]; sdk_path=SDK_PATH)
    path_to_framework(framework) = joinpath(sdk_path, "System/Library/Frameworks/","Accelerate.framework","Frameworks",framework*".framework","Headers")
    path_to_mps_framework(framework) = joinpath(sdk_path, "System/Library/Frameworks/","Accelerate.framework","Frameworks",framework*".framework","Headers")
    ...
    if "all" in names || "libBNNS" in names
        fwpath = path_to_framework("vecLib")
        tctx = wrap("libBNNS", joinpath(fwpath, "vecLib.h"); defines)
        push!(ctxs, tctx)
    end

    # if "all" in names || "veclib" in names
    #     fwpath = path_to_framework("vecLib")
    #     tctx = wrap("vecLib", joinpath(fwpath, "vecLib.h"); defines)
    #     push!(ctxs, tctx)
    # end

    # if "all" in names || "vimage" in names
    #     fwpath = path_to_framework("vImage")
    #     tctx = wrap("vImage", joinpath(fwpath, "vImage.h"); defines)
    #     push!(ctxs, tctx)
    # end
